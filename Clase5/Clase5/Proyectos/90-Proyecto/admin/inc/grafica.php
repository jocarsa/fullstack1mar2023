...
for($i = 0; $i<count($partido);$i++){
    if(explode(":",$partido[$i])[1] > $maximo){$maximo = explode(":",$partido[$i])[1];}
}
$altura = 512 - 10 - 10;
$factor = $altura/$maximo;
for($i = 0; $i<count($partido);$i++){
    imagefilledrectangle($lienzo, $i*$anchurabarra+10, 502-explode(":",$partido[$i])[1]*$factor, $i*$anchurabarra+$anchurabarra-2, 502, $rojo);
    imagettftext(
    $lienzo,
    12,
    0,
