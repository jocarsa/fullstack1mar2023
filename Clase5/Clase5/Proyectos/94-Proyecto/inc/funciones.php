    ...
    while ($fila = mysqli_fetch_assoc($resultado)) {
        echo '
            <article>
                ...
                <h3>'.$fila['titulo'].'</h3>
                <p>'.$fila['texto'].'</p>
                <a href="entrada.php?id='.base64_encode($fila['Identificador']).'">
                <button>Ver artículo</button>
                </a>
            </article>
        ';
...
function entradaSeleccionada(){
    $conexion = mysqli_connect("localhost", "usuariodb", "contrasenadb", "aplicacionweb");
     // Preparo una petición a la base de datos
    $peticion = "
    SELECT * 
    FROM blog
    WHERE Identificador = ".base64_decode($_GET['id'])."
    ";
    ...
    while ($fila = mysqli_fetch_assoc($resultado)) {
        echo '
            <article>
                ...
                <a href="entrada.php?id='.$fila['Identificador'].'">
